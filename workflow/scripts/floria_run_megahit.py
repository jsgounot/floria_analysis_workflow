...
# @Last Modified time: 2023-06-26 18:25:53
...
def make_haplotype_assembly(elements, wthreads=1):
    ...
    wdir = partname + '.megahit'
    subname = os.path.basename(partname)
    outfile = wdir + '.fasta'
    outfile_cp = outfile + '.gz'
    scaffold = os.path.join(wdir, 'megahit', 'final.contigs.fa')

    if os.path.isfile(outfile_cp):
        f'File {outfile_cp} already processed (rerun?), continue ...'
        return outfile_cp

    r1 = partname + '_paired1.megahit.fastq.gz'
    r2 = partname + '_paired2.megahit.fastq.gz'

    assert os.path.isfile(r1)
    assert os.path.isfile(r2)

    logfile = os.path.join(wdir, 'megahit.logfile.txt')
    ...
    if not (isempty(r1) and isempty(r2)):
        cmdline = f'megahit -1 {r1} -2 {r2} -o {wdir}/megahit -t {wthreads}'
